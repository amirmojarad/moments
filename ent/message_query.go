...
func (mq *MessageQuery) QueryPrivateChat() *PrivateChatQuery {
	query := &PrivateChatQuery{config: mq.config}
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withPrivateChat: mq.withPrivateChat.Clone(),
...
func (mq *MessageQuery) WithPrivateChat(opts ...func(*PrivateChatQuery)) *MessageQuery {
	query := &PrivateChatQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withPrivateChat = query
	return mq
}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	var (
		...
		_spec       = mq.querySpec()
		loadedTypes = [2]bool{
			mq.withPrivateChat != nil,
			mq.withOwner != nil,
		}
	)
	if mq.withPrivateChat != nil || mq.withOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withPrivateChat; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Message)
		for i := range nodes {
			if nodes[i].private_chat_chats == nil {
				continue
			}
			fk := *nodes[i].private_chat_chats
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(privatechat.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "private_chat_chats" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.PrivateChat = n
			}
		}
	}
