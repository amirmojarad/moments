...
// The ChannelPostFunc type is an adapter to allow the use of ordinary
// function as ChannelPost mutator.
type ChannelPostFunc func(context.Context, *ent.ChannelPostMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ChannelPostFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ChannelPostMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChannelPostMutation", m)
	}
	return f(ctx, mv)
}
